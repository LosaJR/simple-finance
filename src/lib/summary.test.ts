...
describe('cycle ids', () => {
  ...
  it('keeps February and March separate when a day 31 payday falls on a weekend', () => {
    const payday = getPaydayDate(2027, 1, 31)
    expect([payday.getFullYear(), payday.getMonth(), payday.getDate()]).toEqual([2027, 2, 1])
    expect(getCycleId('2027-02-28', 31)).toBe('2027-02')
